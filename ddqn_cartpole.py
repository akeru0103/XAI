...
def optimize_model():
    ...
    #q_ast = policy_net
    global past_q#q_ast
    #print("id(policy_net) = %i" % id(policy_net))
    #print("id(q_ast) =      %i" % id(q_ast))
    ...
    if steps_done % TRAIN_FREQ == 0:
        ...
        next_state_values[non_final_mask] = past_q(non_final_next_states).max(1)[0].detach()#q_ast(non_final_next_states).max(1)[0].detach()
