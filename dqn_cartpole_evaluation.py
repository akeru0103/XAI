...
def plot_durations():
    ...
    if len(durations_t) >= 11:#100:
        #means = durations_t.unfold(0, 100, 1).mean(1).view(-1)
        #means = torch.cat((torch.zeros(99), means))
        means = durations_t.unfold(0, 11, 1).mean(1).view(-1)
        means = torch.cat((torch.zeros(11), means))
...
def plot_average_saliency():
    plt.figure(4)
    plt.clf()
    ave_sali = torch.tensor(ave_saliency, dtype=torch.float)
    plt.title('Training...')
    plt.xlabel('Step')
    plt.ylabel('Average Saliency score ')
    plt.plot(ave_sali.numpy())
    plt.pause(0.001)  # pause a bit so that plots are updated
    if is_ipython:
        display.clear_output(wait=True)
        display.display(plt.gcf())
...
def save_image(image, save_name, save_type='png'):
    ...
    if image.ndim==3:
        ...
        #保存は[縦,横,色],0~255
        image.save('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/images/'+save_name+'.'+save_type)
    elif image.ndim==2:
...
def save_ndarray_list(ndarray_list, save_name, save_type='npz'):
    ...
    if os.path.exists('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files')==False:
        os.mkdir('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files')
    #filesフォルダの作成
    np.savez_compressed('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files/'+save_name+f'{save_num:0=3}'+'.'+save_type, ndarray_list)
    print(' finished')
...
def save_list(list, save_name, save_type='npz'):
    ...
    if os.path.exists('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files')==False:
        os.mkdir('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files')
    #filesフォルダの作成
    np.savez_compressed('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files/'+save_name+'.'+save_type, list)
    print(' finished')
    ...
    for i in count():
        if os.path.exists('results/result'+str(i+1))==False:
            for j in count():
                #print(i-j)
                if i == j:
                    print(' There are no folders in which evaluation were not generated')
                    sys.exit()
                if os.path.exists('results/result'+str(i-j)+'/evaluation')==False:
                    RESULT_NUMBER = i-j
                    break
            break
...
for epi_num in count():
    ...
    policy_net.load_state_dict(torch.load('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/episode'+str(epi_num)+'.pth'))

    print(policy_net)

    # save_folder = make_lowest_folder()

    with open('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/result.txt', mode='w')as f:
        f.write('\nsettings\n')
    ...
    if SALIENCY_SAVING == True:
        ...
        variables2={"EPISODE_NUMBER":EPISODE_NUMBER, "SAVE_FREQUENCY":SAVE_FREQUENCY, "SALIENCY_ROUGHNESS":SALIENCY_ROUGHNESS, "SAVE_SCREEN":SAVE_SCREEN}
        with open('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/files/variables.pickle', mode='wb') as f:
            pickle.dump(variables2, f)
        print(' finished')
    ...
    for i_episode in range(num_episodes):
        ...
        if ave > ave_max and i_episode >= 11:
            ave_max = ave
        ...
        for t in count():
            ...
            if saliency_save_flag == True: #i_episode=0,5,10...=1,6,11...エピソード目
                ...
                #print(saliency_map_sequence[-1])
                ave_saliency.append(np.mean(saliency_map_sequence[-1]))
            ...
            if done:
                ...
                if saliency_save_flag==True:
                    ...
                    save_ndarrays()
                    plt.figure(4)
                    plt.savefig('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/images/ave_saliency ,ep'+f'{i_episode:0=3}'+'.png')
                else:
                    print(' episode: '+str(i_episode)+' / '+str(EPISODE_NUMBER-1)+', reward: '+str(t+1)+', average/ave_max: '+f'{ave:.2f}'+'/'+f'{ave_max:.2f}')
                saliency_save_flag = decision_of_save(i_episode, average_of_reward, 0, 0, 0, 0)
                ...
                if (i_episode+1) % 10 == 0:
                    plt.figure(2)
                    plt.savefig('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/figure.png')
                    # plt.figure(3)
    ...
    with open('results/result'+str(RESULT_NUMBER)+'/files/episode'+str(epi_num)+'/result.txt', mode='w')as f:
        ...
        f.write('SALIENCY_ROUGHNESS: '+str(SALIENCY_ROUGHNESS)+'\n')
        f.write('\nresults\n')
        f.write('average of reward: '+str(ave)+'\n')
...
f = open('results/result'+str(RESULT_NUMBER)+'/evaluation',mode='w')
f.close()
